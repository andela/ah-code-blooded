...
def login(user=None, verified=False, overrides=None, return_field=None):
    """
    ...
    instead of the entire user object. The verified and overrides fields
    ...
    check_client()
    if user is None:
        user = create_user(verified=verified, overrides=overrides)
...
def create_user(verified=False, overrides=None, return_field=None):
    """
    Creates and registers a random user. If verified is set to True the user will
    be verified. Overrides should be a dictionary that specifies fields to
